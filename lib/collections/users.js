    ...
    Meteor.methods({
        ...
        insertUser: function(user) {
            ...
            if (existingUser == null) {
                ...
                if (!Meteor.users.findOne({ _id: existingUser._id, 'profile.clubIds': club._id })) {
        ...
        removeUser: function(user) {
            ...
            // Send user email
            Meteor.call("sendUserEmail", user.emails[0].address, "You are no longer a member of " + club.name, SSR.render("userRemovedEmailTemplate", { user: user, club: club }));
        },
        ...
        joinClub: function(user, club) {
            ...
            var currentUserCount = Meteor.users.find({ 'profile.clubIds': club._id}).count();
            if (currentUserCount >= club.numberOfUsersLimit) {
                ...
                // Send user email
                Meteor.call("sendUserEmail", user.emails[0].address, "You are now a member of " + club.name, SSR.render("userRemovedEmailTemplate", { user: user, club: club }));
            }
        },
